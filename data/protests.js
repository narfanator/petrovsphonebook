const protests = [
   {
      "name":"Standing Rock",
      "jurisdiction": ["United States", "North Dakota", "County", "Collbone"],
      "id":"1161022966406956503",
      "contacts": {
        "United States": [
          ["White House","<fb link>","<phone line>", "people":[]]
        ],
        "North Dakota": [
          ["Governer's Office", "<fb link>", "<phone line>", "people":[]],
          ["State Troopers", "<fb link>", "<phone line>", "people":[]],
          ["National Guard", "<phone line>", "people":[]],
        ],
        "ACLU": ["..."]
      }
   },
   {
     //TODO: When localizing, also have how jurisdiction list turns into human name - in the US, it's City, State
      "name":"Occupy LA",
      "jurisdiction": ["United States", "California", "Los Angeles", "Los Angeles"],
      "id":"1160844458347054781",
      "contacts": {
        "United States": [
          ["White House","<fb link>","<phone line>", "people":[]]
        ],
        "North Dakota": [
          ["Governer's Office", "<fb link>", "<phone line>", "people":[]],
          ["State Troopers", "<fb link>", "<phone line>", "people":[]],
          ["National Guard", "<phone line>", "people":[]],
        ],
        "ACLU": ["..."]
      }
    }
];

export default protests;
